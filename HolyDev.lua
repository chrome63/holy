...
function ResolveDisplayedWeight(baseWeight)
    ...
    -- Grow a Garden display fallback:
    -- Normal visible KG from Age 1-100 is BaseWeight × 11.
    -- Broken levels above Age 100 do not keep adding more BaseWeight,
    -- so fallback display KG should stay capped at x11.
    local displayWeight =
        baseWeight * 11
...
function ResolveDisplayWeightFromBaseAndAge(baseWeight, age)
    ...
    -- Age scaling should not go above the normal x11 display cap.
    -- Age 1-100 reaches x11.
    -- Age 101-125 should not keep increasing display KG.
    local multiplier =
        11
