...
pcall(function()
    ApplyCompactGameSideButtons()
end)
--==================================================
-- VISUAL PATCH: COMPACT GAME SIDE BUTTONS
-- Shrinks/moves Shop, Trade, and Pass so HOLY HUDs
-- have clean left-side space.
-- No task.wait needed; called after PlayerGui readiness.
--==================================================

function ApplyCompactGameSideButtons()
    ...
    local function StyleButton(button, width, height, yOffset)
    ...
    if sideBtns then
        ...
        if StyleButton(shop, 76, 34, 44) then
            applied =
                true
        end

        if StyleButton(trade, 76, 34, 44) then
            applied =
                true
        end
    end
    ...
    --==================================================
    -- PASS BUTTON
    -- Usually lives separately, so search PlayerGui.
    --==================================================

    for _, obj in ipairs(playerGui:GetDescendants()) do

        if obj:IsA("GuiObject")
        and tostring(obj.Name):lower():find("pass", 1, true) then

            if StyleButton(obj, 96, 42, 44) then
                applied =
                    true
            end
        end
