...
function SniperConfirmPendingBuyByPetName(petName, reason, petId)
    ...
    if bestRecord then
        ...
        print(
            "[HOLY SNIPER]",
            "inventory seen, waiting for sheckles drop",
            "| pet:",
            tostring(petName),
            "| petId:",
            tostring(petId),
...
function SniperMarkPendingAcceptedByKey(key, reason)
    ...
    if SniperState.RequireShecklesDrop == true then
        ...
        or not tonumber(currentSheckles)
        or currentSheckles >= record.StartedSheckles then

            print(
                "[HOLY SNIPER]",
                "ignored accept signal until sheckles drop",
                "| pet:",
                tostring(record.PetName),
                "| uuid:",
                tostring(record.UUID),
    ...
    if record.InventorySeen == true then
        ...
        SniperConfirmPendingBuyRecord(
            record,
            record.InventoryReason or "inventory after sheckles",
