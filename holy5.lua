...
--==================================================
-- [4.53] AUTO COLLECT SEEDS
-- Gold / Rainbow seed prompt collector.
--==================================================

GAG2_SEED_COLLECTOR_STATE =
    GAG2_SEED_COLLECTOR_STATE
    or {
        Enabled = false,
        Running = false,
...
function GAG2SeedCollectorSetStatus(text)
    ...
    if Options.HolyGAG2AutoCollectSeedsStatus then

        Options.HolyGAG2AutoCollectSeedsStatus:SetText(
            '<font color="rgb(196,181,253)"><b>Auto Collect Seeds</b></font>'
            .. '\nEnabled: '
            .. (
                state.Enabled == true
                and "YES"
            ...
            .. ' | Seed: '
            .. tostring(state.SeedType or "Gold")
            .. '\nMove: '
            .. tostring(state.MovementMode or "Teleport")
            .. ' | Collect: '
            .. tostring(state.CollectMode or "Hold")
            .. '\nTarget: '
            .. tostring(state.LastTarget or "None")
            .. '\nCollected: Gold x'
            .. tostring(state.Collected and state.Collected.Gold or 0)
            .. ' | Rainbow x'
            .. tostring(state.Collected and state.Collected.Rainbow or 0)
            .. '\nStatus: '
            .. tostring(state.LastStatus or "Ready.")
        )
    end
...
function GAG2SeedCollectorGetFolder()
...
function GAG2SeedCollectorGetCharacterParts()
...
function GAG2SeedCollectorGetPartPosition(part)
...
function GAG2SeedCollectorFindPrompt(seedPart)

    if typeof(seedPart) ~= "Instance" then
        return nil
    end

    if seedPart:IsA("ProximityPrompt") then
        return seedPart
    end

    for _, descendant in ipairs(seedPart:GetDescendants()) do
...
function GAG2SeedCollectorGetSeedKind(seedPart)
    ...
    local text =
        (
            tostring(seedPart.Name or "")
            .. " "
            .. tostring(objectText)
            .. " "
            .. tostring(seedPart:GetAttribute("SeedType") or "")
            .. " "
            .. tostring(seedPart:GetAttribute("Type") or "")
            .. " "
            .. tostring(seedPart:GetAttribute("Seed") or "")
        ):lower()
    ...
    pcall(function()
        ...
        goldAttr =
            seedPart:GetAttribute("GoldSeed") == true

        rainbowAttr =
            seedPart:GetAttribute("RainbowSeed") == true
    end)

    if rainbowAttr == true
    or text:find("rainbow", 1, true) then

        return "Rainbow"
    end

    if goldAttr == true
    or text:find("gold", 1, true) then
...
function GAG2SeedCollectorKindAllowed(kind)
    ...
    local selected =
        CleanText(
            GAG2_SEED_COLLECTOR_STATE.SeedType
        )
...
function GAG2SeedCollectorCountSeed(kind)
    ...
    local function scan(container)
        ...
        for _, child in ipairs(container:GetChildren()) do
            ...
            if child:IsA("Tool") then
                ...
                local mainCategory =
                    CleanText(
                        child:GetAttribute("MainCategory")
                    )

                if seedName == "" then
                    seedName =
                        CleanText(child.Name)
                end

                if seedName == kind
                and (
                    mainCategory == ""
                    or mainCategory == "Seed"
                ) then
    ...
    scan(backpack)
    scan(LOCAL_PLAYER.Character)

    return count
end

function GAG2SeedCollectorFindNearestTarget()

    local folder =
        GAG2SeedCollectorGetFolder()
    ...
    local _, root =
        GAG2SeedCollectorGetCharacterParts()
    ...
    for _, child in ipairs(folder:GetChildren()) do
        ...
        local kind =
            GAG2SeedCollectorGetSeedKind(
                child
            )

        if GAG2SeedCollectorKindAllowed(kind) == true then
            ...
            local position =
                GAG2SeedCollectorGetPartPosition(
                    child
                )
...
function GAG2SeedCollectorGetSafeCFrame(position)
    ...
    local finalPosition =
        groundPosition + Vector3.new(0, 3.25, 0)
...
function GAG2SeedCollectorMoveToTarget(target)

    if type(target) ~= "table"
    or typeof(target.Position) ~= "Vector3" then
    ...
    local mode =
        CleanText(
            GAG2_SEED_COLLECTOR_STATE.MovementMode
        )
    ...
    if mode == "Walk" then
        ...
        while GAG2_SEED_COLLECTOR_STATE.Enabled == true
        and target.Part
        and target.Part.Parent
        and os.clock() - startedAt < 12 do

            if (root.Position - target.Position).Magnitude <= 8.5 then
                return true
            end
        ...
        return (
            target.Part
            and target.Part.Parent
            and (root.Position - target.Position).Magnitude <= 10
        )
    end

    local cframe =
        GAG2SeedCollectorGetSafeCFrame(
            target.Position
        )
...
function GAG2SeedCollectorFirePrompt(prompt)
    ...
    if CleanText(GAG2_SEED_COLLECTOR_STATE.CollectMode) == "Instant" then
    ...
    local holdSeconds =
        math.clamp(
            tonumber(GAG2_SEED_COLLECTOR_STATE.HoldSeconds)
            or 3.2,
            0.25,
            15
        )
...
function GAG2SeedCollectorWaitForConfirm(kind, beforeCount, seedPart)
    ...
    while os.clock() - startedAt < timeout do
        ...
        if GAG2SeedCollectorCountSeed(kind) > beforeCount then
            return true, "backpack confirmed"
        end

        if seedPart
        and seedPart.Parent == nil then

            task.wait(0.35)

            if GAG2SeedCollectorCountSeed(kind) > beforeCount then
                return true, "removed + backpack confirmed"
            end
        end
    ...
    return false,
        "no backpack confirmation"
end

function GAG2SeedCollectorCollectTarget(target)

    if type(target) ~= "table"
    or not target.Part
    or not target.Prompt
    or target.Part.Parent == nil then
    ...
    state.LastTarget =
        ...
        .. tostring(
            math.floor(
                tonumber(target.Distance)
                or 0
            )
        )
        .. " studs"

    GAG2SeedCollectorSetStatus(
        "Moving to seed..."
    )

    local moved =
        GAG2SeedCollectorMoveToTarget(
            target
        )
    ...
    if target.Part.Parent == nil then
        ...
        GAG2SeedCollectorSetStatus(
            "Seed disappeared before collect."
        )
    ...
    GAG2SeedCollectorFirePrompt(
        target.Prompt
    )

    local confirmed, reason =
        GAG2SeedCollectorWaitForConfirm(
            kind,
            beforeCount,
            target.Part
        )
    ...
    if confirmed == true then
        ...
        state.Collected[kind] =
            tonumber(state.Collected[kind])
            or 0

        state.Collected[kind] =
            state.Collected[kind] + 1
...
function GAG2SeedCollectorStart()
    ...
    task.spawn(function()
        ...
        and state.LoopToken == token do
            ...
            if state.Busy ~= true then
                ...
                local target =
                    GAG2SeedCollectorFindNearestTarget()

                if target then

                    GAG2SeedCollectorCollectTarget(
                        target
                    )
...
function GAG2SeedCollectorStop(skipDirty)
...
function GAG2SeedCollectorSetEnabled(value)

    local enabled =
        value == true

    GAG2_SEED_COLLECTOR_STATE.Enabled =
        enabled
    ...
    if enabled == true then
        ...
        GAG2SeedCollectorStart()

    else

        GAG2SeedCollectorStop(
            true
        )
    end
...
function GAG2SeedCollectorSetSeedType(value)
...
function GAG2SeedCollectorSetMovementMode(value)

    local cleaned =
        CleanText(value)

    if cleaned ~= "Teleport"
    and cleaned ~= "Walk" then
...
function GAG2SeedCollectorSetCollectMode(value)

    local cleaned =
        CleanText(value)

    if cleaned ~= "Instant"
    and cleaned ~= "Hold" then

        cleaned =
            "Hold"
    end

    GAG2_SEED_COLLECTOR_STATE.CollectMode =
        cleaned
...
function GAG2SeedCollectorSetHoldSeconds(value)
    ...
    GAG2_SEED_COLLECTOR_STATE.HoldSeconds =
        seconds
...
function GAG2SeedCollectorRestoreFromOptions()

    task.defer(function()

        if Options.HolyGAG2SeedCollectorType then

            GAG2_SEED_COLLECTOR_STATE.SeedType =
                CleanText(
                    Options.HolyGAG2SeedCollectorType.Value
                )
        end

        if Options.HolyGAG2SeedCollectorMovement then

            GAG2_SEED_COLLECTOR_STATE.MovementMode =
                CleanText(
                    Options.HolyGAG2SeedCollectorMovement.Value
                )
        end

        if Options.HolyGAG2SeedCollectorCollectMode then

            GAG2_SEED_COLLECTOR_STATE.CollectMode =
                CleanText(
                    Options.HolyGAG2SeedCollectorCollectMode.Value
                )
        end

        if Options.HolyGAG2SeedCollectorHoldSeconds then

            GAG2_SEED_COLLECTOR_STATE.HoldSeconds =
                math.clamp(
                    tonumber(
                        Options.HolyGAG2SeedCollectorHoldSeconds.Value
                    )
                    or 3.2,
                    0.25,
                    15
...
local ShopsSeedCollectorBox =
    ShopsTab:AddLeftGroupbox(
        "Auto Collect Seeds"
    )

ShopsSeedCollectorBox:AddToggle("HolyGAG2AutoCollectSeeds", {
    Text = "Auto Collect Seeds",
    Default = false,
    Tooltip = "Collects Gold/Rainbow weather seeds from workspace.Map.SeedPackSpawnServerLocations.",
    Callback = function(value)

        GAG2SeedCollectorSetEnabled(
            value == true
        )
    end,
})

ShopsSeedCollectorBox:AddDropdown(
    "HolyGAG2SeedCollectorType",
    {
        Text = "Seed Type",
        Values = {
            "Gold",
        ...
        Tooltip = "Which weather seed type to collect.",
    }
):OnChanged(function(value)

    GAG2SeedCollectorSetSeedType(
        value
    )
end)

ShopsSeedCollectorBox:AddDropdown(
    "HolyGAG2SeedCollectorMovement",
    {
        Text = "Movement Mode",
        Values = {
            "Teleport",
        ...
        Tooltip = "Teleport is fastest. Walk is safer but slower.",
    }
):OnChanged(function(value)

    GAG2SeedCollectorSetMovementMode(
        value
    )
end)

ShopsSeedCollectorBox:AddDropdown(
    "HolyGAG2SeedCollectorCollectMode",
    {
        Text = "Collect Mode",
        Values = {
            "Hold",
        ...
        Searchable = false,
        MaxVisibleDropdownItems = 2,
        Tooltip = "Hold matches the real 3 second claim prompt. Instant uses fireproximityprompt directly.",
    }
):OnChanged(function(value)

    GAG2SeedCollectorSetCollectMode(
        value
    )
end)

ShopsSeedCollectorBox:AddInput(
    "HolyGAG2SeedCollectorHoldSeconds",
    {
        Text = "Hold Duration",
        Default = "3.2",
        Numeric = true,
        Finished = true,
        Tooltip = "Prompt hold time. Default 3.2 because the real prompt is 3 seconds.",
        Callback = function(value)

            GAG2SeedCollectorSetHoldSeconds(
                value
            )
        end,
    }
...
ShopsSeedCollectorBox:AddLabel("HolyGAG2AutoCollectSeedsStatus", {
    Text =
        '<font color="rgb(196,181,253)"><b>Auto Collect Seeds</b></font>'
        .. '\nEnabled: NO | Seed: Gold'
        .. '\nMove: Teleport | Collect: Hold'
        .. '\nTarget: None'
        .. '\nCollected: Gold x0 | Rainbow x0'
        .. '\nStatus: Ready.',
    DoesWrap = true,
})

GAG2SeedCollectorRestoreFromOptions()
