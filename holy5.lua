    ...
    while os.clock() - started < limit do
        ...
        if typeof(targetPosition) == "Vector3" then
            ...
            and root then
                ...
                if (root.Position - closePosition).Magnitude > 1.5 then
...
function SniperAttemptBuyEntry(entry)
    ...
    if type(SniperHasUnacceptedPendingBuy) == "function"
        ...
        local okTask, taskError =
            xpcall(
                function()
                    ...
                    local acceptedAfterPrompt =
                        SniperWaitForBuyAccepted(
                            entry,
                            0.35
                        )

                    local okBuy =
                        acceptedAfterPrompt == true

                    local info =
                        acceptedAfterPrompt == true
                        and "accepted after prompt"
                        or "not fired yet"

                    if acceptedAfterPrompt ~= true then
                    ...
                    if okBuy == true
                    or promptOk == true then

                        SetSniperStatus(
                            "Buy sent, waiting accept: "
                            .. tostring(entry.Name)
                        )
                        ...
                        if accepted == true then
                            ...
                            warn(
                                "[HOLY SNIPER]",
                                "buy packet/prompt did not get accepted",
                                "| pet:",
                                tostring(entry.Name),
                                "| uuid:",
                                tostring(entry.UUID),
