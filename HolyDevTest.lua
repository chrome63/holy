...
--==================================================
-- BOOTH AUTO CLAIM (ONE-SHOT, AUTHORITATIVE)
--==================================================
function ExecuteBoothClaim()
    if BoothAuto.InProgress or not BoothAuto.Enabled then
        return
    end

    if game.PlaceId ~= TRADING_WORLD_PLACE_ID then
        warn("[Booth] Not in Trade World")
        BoothAuto.Enabled = false
        return
    end

    BoothAuto.InProgress = true
    print("[Booth] Fetching booth data...")

    local data = LatestBoothData

    if not data or not data.Booths then
        warn("[Booth] Failed to get booth data")
        BoothAuto.Enabled = false
        BoothAuto.InProgress = false
        return
    end

    local boothsFolder = workspace:FindFirstChild("TradeWorld")
        and workspace.TradeWorld:FindFirstChild("Booths")
...
--==================================================
-- FIND FREE BOOTH CLOSEST TO PRIORITY POINT
-- Priority point:
-- workspace.TradeWorld.Model:GetChildren()[5]:GetChildren()[8]
--==================================================
...
if not priorityPosition then
    warn("[Booth] Priority point missing, falling back to first free booth")
end
    ...
    if boothInfo.Owner == nil then
        ...
        if model then
            ...
            if priorityPosition then
                ...
                -- fallback if priority point cannot resolve
                targetBooth = model
                break
            end
        end
...
if targetBooth and priorityPosition then
    print(
        "[Booth] Closest free booth selected:",
        targetBooth.Name,
        "distance:",
        tostring(math.floor(bestDistance))
    )
end

    if not targetBooth then
        warn("[Booth] No free booth found")
        BoothAuto.Enabled = false
        BoothAuto.InProgress = false
        return
    end

    print("[Booth] Claiming:", targetBooth.Name)
    ...
    local ClaimBooth = GameEvents
        :WaitForChild("TradeEvents")
        :WaitForChild("Booths")
        :WaitForChild("ClaimBooth")

    local EquipSkin = GameEvents
        :WaitForChild("TradeBoothSkinService")
        :WaitForChild("Equip")

    --==================================================
    -- CLAIM SEQUENCE (CONFIRMED VIA SPY)
    -- Uses selected owned booth skin only.
    --==================================================
...
print("[Booth] Claim attempt finished")

--==================================================
-- AUTO TELEPORT (DELAYED + VERIFIED)
--==================================================
if BoothAuto.AutoTeleport then
    task.spawn(function()

        -- Custom booth skins can rebuild the stand model after EquipSkin.
        -- Wait slightly longer before resolving booth geometry.
        task.wait(1.75)
...
if Library and Library.Options and Library.Options.AutoClaimBooth then
...
function ArmListingsAutostartFromSavedToggle()
    ...
    if type(CountActiveListingFilters) == "function" then

        filterCount =
            CountActiveListingFilters()

    elseif type(ListingsState.ListingFilters) == "table" then
    ...
    -- No saved listing filters = do not auto-start.
    ...
    if not configAllowed then
        ...
        warn(
            "[LISTINGS] Saved filters autostart blocked:",
    ...
    --==================================================
    -- OWN BOOTH LISTING SYNC GATE
    -- Never auto-start AutoList until the booth store has
    -- confirmed which inventory UUIDs are already listed.
    --==================================================

    local syncReady, syncReason =
        WaitForOwnListedUUIDSync(12)
    ...
    if not syncReady then
        ...
        warn(
            "[LISTINGS] Saved filters autostart delayed:",
            tostring(syncReason)
        )
        ...
        task.delay(3, function()
            ...
            if type(ArmListingsAutostartFromSavedToggle) == "function" then
                ArmListingsAutostartFromSavedToggle()
            end
        end)
    ...
    --==================================================
    -- SYNC UI TOGGLE VISUALLY
    -- This makes Start AutoList show ON after autostart.
    --==================================================
...
if not syncReady then
    ...
    "| Filter:",
    tostring(matchedFilter and matchedFilter.MinWeight or "?"),
    "-",
    tostring(matchedFilter and matchedFilter.MaxWeight or "?"),
    "| Price:",
    tostring(matchedFilter and matchedFilter.Price or ListingsState.Price),
    "| UUID:",
    tostring(pet.UUID)
)
                ...
                local minWeight =
                    tonumber(matchedFilter and matchedFilter.MinWeight)

                local maxWeight =
                    tonumber(matchedFilter and matchedFilter.MaxWeight)
            ...
            local summary =
    string.format(
        "[LISTINGS] Pass complete | matched: %s | queued: %s | booth-skip: %s | runtime-skip: %s | failed-skip: %s | pending-skip: %s | queued-skip: %s | queue: %s",
        tostring(matched),
        tostring(queued),
        ...
        tostring(#ListingsState.ListingQueue)
            ...
            or not ListingsState.QuietWhenComplete then
                ...
                PrintListingSummaryOnce(
                    string.format(
                        "[LISTINGS] Idle | matched: %s | already listed: %s | runtime listed: %s | failed: %s | pending: %s | queue: %s",
                    tostring(matched),
                    tostring(skippedBoothListed),
                    tostring(skippedRuntimeListed),
                    tostring(skippedFailed),
