...
-- Keeps unexpected non-HOLY warnings visible.
--==================================================
...
--==================================================
-- DEV PRODUCT LEAK TRACKER
-- Detects new in-game Developer Products and price changes.
-- First run creates a silent baseline so it does not spam old products.
--==================================================

DevProductLeakWebhook = {
    Enabled = true,

    -- Put your dev product leak webhook here.
...
function ResolveDevProductRequestFunction()
...
function IsDevProductKey(key)

    key =
        tostring(key or ""):lower()

    return key == "purchaseid"
        or key == "productid"
        or key == "product_id"
        or key == "devproductid"
        or key == "developerproductid"
end

function SafeRequireDevProductModule(module)
...
function AddDevProductCandidate(candidates, id, source, path, itemName)

    id =
        tonumber(id)

    if not id
    or id < 100000000 then
        return false
    end
    ...
    if candidates[key].ItemName == ""
    and itemName then
        candidates[key].ItemName =
            tostring(itemName)
    end

    if candidates[key].Source == ""
    and source then
        candidates[key].Source =
            tostring(source)
    end

    if candidates[key].Path == ""
    and path then
        candidates[key].Path =
            tostring(path)
    end
...
function ScanDevProductTable(value, source, path, candidates, depth, nearestName)
    ...
    for key, child in pairs(value) do
        ...
        if IsDevProductKey(keyText) then

            AddDevProductCandidate(
                candidates,
                child,
                source,
                nextPath,
                nearestName or keyText
            )
...
function ScanDevProductCandidates()
...
function FormatDevProductRobux(value)
...
function ResolveDevProductStatus(price)
...
function ResolveDevProductInfo(candidate)
    ...
    if not ok
    or type(info) ~= "table"
    or not info.Name then
        return nil
    end

    local price =
        tonumber(info.PriceInRobux)
    ...
    return {
        ...
        PriceInRobux = price,
        IconImageAssetId = tonumber(info.IconImageAssetId) or 0,
        Created = tostring(info.Created or ""),
        ItemName = tostring(candidate.ItemName or ""),
        Source = tostring(candidate.Source or ""),
        Path = tostring(candidate.Path or ""),
        Status = ResolveDevProductStatus(price),
        LastSeen = os.time(),
    }
end

function LoadDevProductLeakSnapshot()
    ...
    local file =
        DevProductLeakWebhook.SnapshotFile
...
function SaveDevProductLeakSnapshot(products)
    ...
    if not ok then
        ...
        warn(
            "[DEV LEAKS] Snapshot save failed:",
            tostring(err)
        )
...
function QueueDevProductLeakWebhook(event)

    if type(event) ~= "table" then
        return false
    end

    DevProductLeakWebhook.Queue =
        DevProductLeakWebhook.Queue
        or {}
...
function BuildDevProductThumbnail(product)
...
function SendDevProductLeakWebhookNow(event)

    if type(DevProductLeakWebhook) ~= "table"
    or DevProductLeakWebhook.Enabled ~= true then
        return false
    end

    if type(event) ~= "table"
    or type(event.Product) ~= "table" then
        return false
    end

    local webhookUrl =
        tostring(DevProductLeakWebhook.URL or "")
            :gsub("%s+", "")

    if webhookUrl == ""
    or webhookUrl == "PUT_DEV_PRODUCT_LEAK_WEBHOOK_HERE" then
        warn("[DEV LEAKS] Webhook URL missing")
        return false
    end

    local requestFunction =
        ResolveDevProductRequestFunction()

    if not requestFunction then
        warn("[DEV LEAKS] No request function available")
        return false
    end
    ...
    if eventType == "PriceChanged" then
        title =
    ...
    local description =
        "**"
        .. tostring(product.Name or "Unknown Product")
        .. "**"
        .. "\n\n"
        .. "**Product ID**\n"
    ...
    if eventType == "PriceChanged" then
        ...
        description =
            description
            .. "**Old Price**\n"
            .. FormatDevProductRobux(event.OldPrice)
            .. " Robux"
            .. "\n\n"
            .. "**New Price**\n"
            .. FormatDevProductRobux(product.PriceInRobux)
            .. " Robux"
            .. "\n\n"
    ...
    description =
        description
        .. "**Status**\n"
        .. tostring(product.Status or "Unknown")
        .. "\n\n"
        .. "**Item**\n"
        .. tostring(product.ItemName ~= "" and product.ItemName or "Unknown")
        .. "\n\n"
        .. "**Source**\n"
        .. "```lua\n"
        .. tostring(product.Path ~= "" and product.Path or product.Source)
        .. "\n```"

    if tostring(product.Created or "") ~= "" then

        description =
            description
            .. "\n"
            .. "**Created**\n"
            .. tostring(product.Created)
    end
    ...
    local thumbnail =
        BuildDevProductThumbnail(product)
    ...
    if not ok then
        warn("[DEV LEAKS] Webhook send failed:", tostring(response))
        return false
    end
...
function ProcessDevProductLeakWebhookQueue()

    if DevProductLeakWebhook.Sending then
        return
    end

    DevProductLeakWebhook.Sending =
        true

    task.spawn(function()

        while #DevProductLeakWebhook.Queue > 0 do

            local delayTime =
                SafeNumber(
                    DevProductLeakWebhook.SendDelay,
                    1.25
                )
            ...
            local elapsed =
                os.clock()
                - SafeNumber(
                    DevProductLeakWebhook.LastSend,
                    0
                )
            ...
            SendDevProductLeakWebhookNow(event)
...
function RunDevProductLeakCheck()

    if DevProductLeakWebhook.Checking then
        return
    end

    DevProductLeakWebhook.Checking =
        true
    ...
    local ok, err =
        pcall(function()
            ...
            local candidates =
                ScanDevProductCandidates()

            local productIds =
                {}

            for productId in pairs(candidates) do
                table.insert(
                    productIds,
                    tonumber(productId)
                )
            ...
            local priceChangeCount =
                0

            for _, productId in ipairs(productIds) do

                local candidate =
                    candidates[tostring(productId)]

                local product =
                    ResolveDevProductInfo(candidate)
                ...
                if product then
                    ...
                    if not firstRun then
                        ...
                        if type(old) ~= "table" then
                            ...
                            QueueDevProductLeakWebhook({
                                Type = "New",
                                Product = product,
                            })

                            print(
                                "[DEV LEAKS] New product:",
                                tostring(product.Name),
                                "|",
                                tostring(product.ProductId),
                                "|",
                                tostring(product.PriceInRobux)
                            )

                        elseif tonumber(old.PriceInRobux)
                            ~= tonumber(product.PriceInRobux)
                        then
                            ...
                            QueueDevProductLeakWebhook({
                                Type = "PriceChanged",
                                Product = product,
                                OldPrice = tonumber(old.PriceInRobux),
                            })

                            print(
                                "[DEV LEAKS] Price changed:",
                                tostring(product.Name),
                                "|",
                                tostring(old.PriceInRobux),
                                "->",
                                tostring(product.PriceInRobux)
                            )
                        end
            ...
            SaveDevProductLeakSnapshot(newProducts)

            if firstRun then

                print(
                    "[DEV LEAKS] Baseline saved silently | candidates:",
                    tostring(#productIds),
                    "| valid:",
                    tostring(validCount)
                )
                ...
                print(
                    "[DEV LEAKS] Check complete | candidates:",
                    tostring(#productIds),
                    "| valid:",
                    tostring(validCount),
                    "| new:",
                    tostring(newCount),
                    "| price changed:",
                    tostring(priceChangeCount)
                )

                ProcessDevProductLeakWebhookQueue()
            end
        end)
    ...
    if not ok then
        warn("[DEV LEAKS] Check failed:", tostring(err))
    end
end

function StartDevProductLeakTracker()

    if DevProductLeakWebhook.Running then
        return
    end

    DevProductLeakWebhook.Running =
        true
    ...
    task.spawn(function()
        ...
        while DevProductLeakWebhook.Enabled == true do
            ...
            RunDevProductLeakCheck()

            local interval =
                SafeNumber(
                    DevProductLeakWebhook.CheckInterval,
                    300
                )
...
-- Uses ReplicatedStorage.Data.PetRegistry.PetList[PetName].Icon
--==================================================
...
function ResolveMarketTrackerPetRegistry()
    ...
    local petRegistryModule =
        dataFolder
        and dataFolder:FindFirstChild("PetRegistry")

    if not petRegistryModule
    or not petRegistryModule:IsA("ModuleScript") then
        return nil
    end
...
function ExtractAssetIdFromIcon(icon)
...
function BuildRobloxAssetThumbnailUrl(assetId)
...
function ResolveMarketTrackerPetThumbnail(listing)
    ...
    if MarketTrackerPetIconCache[petName] ~= nil then
        return MarketTrackerPetIconCache[petName]
    end

    local registry =
        ResolveMarketTrackerPetRegistry()

    local petData =
        registry
        and registry.PetList
        and registry.PetList[petName]

    if type(petData) ~= "table" then
        MarketTrackerPetIconCache[petName] = false
        return nil
    end

    local assetId =
        ExtractAssetIdFromIcon(
            petData.Icon
    ...
    local thumbnail =
        BuildRobloxAssetThumbnailUrl(assetId)
...
function SendMarketTrackerWebhookNow(listing)
    ...
    local payload = {
        embeds = {{
            ...
            local petThumbnail =
    ResolveMarketTrackerPetThumbnail(listing)
    ...
    tostring(priority)
        ...
        HolyNotify(
            ...
            tostring(pet)
                ...
                .. tostring(saveTarget)
                .. " using "
                .. tostring(weightMode == "BaseWeight" and "BaseWeight" or "Display KG")
                .. ".",
            "plus",
            3
        )
